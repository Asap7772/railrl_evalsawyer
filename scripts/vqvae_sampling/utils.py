...
import time
import os
from datasets import BlockDataset, LatentBlockDataset
import numpy as np
...
def load_latent_block(filepath):
    train = LatentBlockDataset(filepath, train=True,
                         transform=None)

    val = LatentBlockDataset(filepath, train=False,
                       transform=None)
    return train, val
...
def load_data_and_data_loaders(filepath, dataset, batch_size):
    if dataset == 'CIFAR10':
