...
path_func = lambda name: '/media/ashvin/data2/data/baseline/'+ name
...
all_demos = [
    ...
    dict(path=path_func('fixed_pot_extra1_demos.npy'), obs_dict=True, is_demo=True, data_split=0.2),
...
if __name__ == "__main__":
    variant = dict(
        ...
        trainer_kwargs=dict(
            ...
            q_num_pretrain2_steps=25001, #25001 #HERE
        ...
        algo_kwargs=dict(
            ...
            num_epochs=101, #1001
            num_eval_steps_per_epoch=150, #500
        ...
        presampled_goal_kwargs=dict(
            eval_goals='/media/ashvin/data2/data/val/v1/ccvae_pot1_eval_goals.pkl',
            expl_goals=None,
        ),
        ...
        pretrained_vae_path=path_func('vae.pt'),
        pretrained_algo_path=path_func('agent_sparse_1.pt'), #agent_sparse_1.pt, agent_sparse_2.pt, agent_dense.pt
    )
    ...
    search_space = {
        ...
        'reward_kwargs.reward_type': ['sparse',], # TRY SPARSE (EPS=1), SPARSE (EPS=2), DENSE (PROB NOT GONNA WORK)
    ...
    for variant in sweeper.iterate_hyperparameters():
        if 'sparse' in variant['pretrained_algo_path']:
            variant['qf_kwargs']['output_activation'] = Clamp(max=0)

        if variant['pretrained_algo_path'] == path_func('agent_sparse_1.pt'):
            variant['reward_kwargs'] == dict(reward_type='sparse', epsilon=1.0)
        if variant['pretrained_algo_path'] == path_func('agent_sparse_2.pt'):
            variant['reward_kwargs'] == dict(reward_type='sparse', epsilon=2.0)
        if variant['pretrained_algo_path'] == path_func('agent_dense.pt'):
            variant['reward_kwargs'] == dict(reward_type='dense', epsilon=1.0)
        
        variants.append(variant)

    run_variants(awac_rig_experiment, variants, run_id=10) #HERE
