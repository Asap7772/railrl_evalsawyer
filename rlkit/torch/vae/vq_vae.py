...
class VQ_VAE(nn.Module):
    ...
    def sample_prior(self, batch_size, cond=None, image_cond=False, reproject=True):
        assert cond is not None
        if image_cond:
            cond = self.encode_np(cond)
        ...
        if reproject: z_q = self.encode(self.decode(z_q))
...
class VAE(nn.Module):
    ...
    def sample_prior(self, batch_size, reproject=True):
        z_s = ptu.randn(batch_size, self.representation_size)
        if reproject: z_s = self.encode(self.decode(z_s))
...
class CCVAE(nn.Module):
    ...
    def sample_prior(self, batch_size, cond=None, image_cond=False, reproject=True):
        ...
        if reproject:
            x_cond = self.decode(z_cond, cond=True)
            z_cat = self.encode(self.decode(z_cat), x_cond)
...
#     ConcatMlp, TanhMlpPolicy, MlpPolicy, Reshape,
#     Flatten,
# )
# from rlkit.torch.networks import CNN, TwoHeadDCNN, DCNN
