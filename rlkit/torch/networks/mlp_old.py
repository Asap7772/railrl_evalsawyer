...
class Mlp(PyTorchModule):
    def __init__(
        ...
        for i, next_size in enumerate(hidden_sizes):
            ...
            if self.normalization_type == 'batch':
                self.conv_norm_layers.append(nn.BatchNorm1d(in_size))
            if self.normalization_type == 'layer':
                self.conv_norm_layers.append(nn.LayerNorm(in_size))
    ...
    def forward(self, input, return_preactivations=False, return_h=False):
        h = input
        for i, fc in enumerate(self.fcs):
            if hasattr(self, 'dense'):  # TODO undo backwards compatibility hack
                if self.dense and i > 0:
...
class MultiHeadedMlp(Mlp):
    ...
    def forward(self, input, **kwargs):
        flat_outputs = super().forward(input, **kwargs)
